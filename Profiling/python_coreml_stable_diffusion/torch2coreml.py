...
from python_coreml_stable_diffusion import controlnet, unet
...
def _save_mlpackage(model, output_path):
    ...
    ct.models.MLModel(
        model._spec, weights_dir=model._weights_dir, is_temp_package=True
    ).save(output_path)
...
def bundle_resources_for_swift_cli(args):
    ...
    if args.convert_controlnet:
        for controlnet_model_version in args.convert_controlnet:
            ...
            target_name = "".join(
                [word.title() for word in re.split("_|-", controlnet_model_name)]
            )

            if os.path.exists(source_path):
                target_path = _compile_coreml_model(
                    source_path, controlnet_dir, target_name
                )
                logger.info(f"Compiled {source_path} to {target_path}")
            else:
                logger.warning(
...
def convert_text_encoder(pipe, args):
    ...
    sample_text_encoder_inputs_spec = {
        k: (v.shape, v.dtype) for k, v in sample_text_encoder_inputs.items()
    }
    ...
    class TextEncoder(nn.Module):
        def __init__(self):
            ...
            setattr(
                ...
                MethodType(_build_causal_attention_mask, self.text_encoder.text_model),
            )
    ...
    coreml_text_encoder.input_description[
        "input_ids"
    ] = "The token ids that represent the input text"
...
def convert_vae_decoder(pipe, args):
    ...
    sample_vae_decoder_inputs = {"z": torch.rand(*z_shape, dtype=torch.float16)}
    ...
    coreml_vae_decoder, out_path = _convert_to_coreml(
        "vae_decoder", traced_vae_decoder, sample_vae_decoder_inputs, ["image"], args
    )
    ...
    # Set the input descriptions
    coreml_vae_decoder.input_description[
        "z"
    ...
    if args.check_output_correctness:
        baseline_out = baseline_decoder(
            z=sample_vae_decoder_inputs["z"].to(torch.float32)
        ).numpy()
        coreml_out = list(
        ...
        report_correctness(
            baseline_out, coreml_out, "vae_decoder baseline PyTorch to baseline CoreML"
        )
...
def convert_vae_encoder(pipe, args):
    ...
    sample_vae_encoder_inputs = {"z": torch.rand(*z_shape, dtype=torch.float16)}
    ...
    coreml_vae_encoder, out_path = _convert_to_coreml(
        "vae_encoder", traced_vae_encoder, sample_vae_encoder_inputs, ["latent"], args
    )
    ...
    # Set the input descriptions
    coreml_vae_encoder.input_description[
        "z"
    ...
    coreml_vae_encoder.output_description[
        "latent"
    ] = "The latent embeddings from the unet model from the input image."
    ...
    if args.check_output_correctness:
        baseline_out = baseline_encoder(
            z=sample_vae_encoder_inputs["z"].to(torch.float32)
        ).numpy()
        coreml_out = list(
        ...
        report_correctness(
            baseline_out, coreml_out, "vae_encoder baseline PyTorch to baseline CoreML"
        )
...
def convert_unet(pipe, args):
    ...
    # If original Unet does not exist, export it from PyTorch+diffusers
        ...
        print(sample_shape)
        print(sample_unet_inputs["timestep"].shape)
        print(encoder_hidden_states_shape)
        ...
        if args.unet_support_controlnet:
            ...
            for down_block in reference_unet.down_blocks:
                ...
                if (
                    hasattr(down_block, "downsamplers")
                    and down_block.downsamplers is not None
                ):
            ...
            additional_residuals_shapes.append(
                (
                    batch_size,
                    reference_unet.mid_block.resnets[-1].out_channels,
                    out_h,
                    out_w,
                )
            )
        ...
        reference_unet = torch.jit.trace(
            reference_unet, list(sample_unet_inputs.values())
        )
        logger.info("Done.")

        if args.check_output_correctness:
            baseline_out = pipe.unet(**baseline_sample_unet_inputs, return_dict=False)[
                0
            ].numpy()
            reference_out = reference_unet(*sample_unet_inputs.values())[0].numpy()
            report_correctness(
                baseline_out, reference_out, "unet baseline to reference PyTorch"
            )
        ...
        coreml_sample_unet_inputs = {
            k: v.numpy().astype(np.float16) for k, v in sample_unet_inputs.items()
        }
        ...
        # Parity check PyTorch vs CoreML
        if args.check_output_correctness:
            coreml_out = list(coreml_unet.predict(coreml_sample_unet_inputs).values())[
                0
            ]
            report_correctness(
                baseline_out, coreml_out, "unet baseline PyTorch to reference CoreML"
            )
...
def convert_safety_checker(pipe, args):
    ...
    sample_image = np.random.randn(1, im_h, im_w, 3).astype(  # B  # H  # w  # C
        np.float32
    )
    ...
    sample_safety_checker_inputs_spec = {
        k: (v.shape, v.dtype) for k, v in sample_safety_checker_inputs.items()
    }
    ...
    def forward_coreml(self, clip_input, images, adjustment):
        ...
        has_nsfw_concepts = (
            concept_scores.gt(0).float().sum(dim=1).gt(0)[:, None, None, None]
        )
    ...
    # Set the input descriptions
    coreml_safety_checker.input_description[
        "clip_input"
    ...
    if args.check_output_correctness:
        ...
        coreml_out = coreml_safety_checker.predict(coreml_sample_safety_checker_inputs)[
            "concept_scores"
        ]
        report_correctness(
            baseline_out,
...
def convert_controlnet(pipe, args):
    ...
    for i, controlnet_model_version in enumerate(args.convert_controlnet):
        ...
        if (
            base_model is None
            and args.model_version != "runwayml/stable-diffusion-v1-5"
        ):
            logger.warning(
        ...
        if i == 0:
            ...
            sample_controlnet_inputs_spec = {
                k: (v.shape, v.dtype) for k, v in sample_controlnet_inputs.items()
            }
            logger.info(
        ...
        original_controlnet = ControlNetModel.from_pretrained(
            controlnet_model_version, use_auth_token=True
        )
        reference_controlnet = controlnet.ControlNetModel(
        ...
        # JIT trace
        logger.info("JIT tracing..")
        reference_controlnet = torch.jit.trace(
            reference_controlnet, list(sample_controlnet_inputs.values())
        )
        logger.info("Done.")

        if args.check_output_correctness:
            baseline_out = original_controlnet(
                **baseline_sample_controlnet_inputs, return_dict=False
            )
        ...
        # Set the input descriptions
        coreml_controlnet.input_description[
            "sample"
        ] = "The low resolution latent feature maps being denoised through reverse diffusion"
        coreml_controlnet.input_description[
            "timestep"
...
def parser_spec():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If specified, quantize 16-bits weights to 8-bits weights in-place for all models. "
