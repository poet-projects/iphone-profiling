...
class BertForQuestionAnswering {
    ...
    enum Model {
        ...
        case classifier(bert_classifier)
    }
    let model: Model
...
//        self.model = try! .classifier(bert_classifier())
    }
    
    init(_ model: bert_classifier) {
    ...
    func predict(question: String, context: String) -> (start: Int, end: Int, tokens: [String], answer: String) {
        ...
        case .classifier(let m):
            let input = featurizeTokensClassifier(question: question)
...
//
            let tokenIds = Array(
                MLMultiArray.toIntArray(output.var_601)
            )
...
//            let answer = tokenizer.convertWordpieceToBasicTokenList(tokens)
            ...
            return (start: 0, end: 5, tokens: [], answer: tokenIds.map(String.init).joined(separator: "," ))
        }
        }
    ...
    func featurizeTokensClassifier(question: String) -> bert_classifierInput {
        print("featurizing tokens for classifier")
        let tokensQuestion = tokenizer.tokenizeToIds(text: question).prefix(20)

        let nPadding = 22 - tokensQuestion.count - 2
        /// Sequence of input symbols. The sequence starts with a start token (101) followed by question tokens that are followed be a separator token (102) and the document tokens.The document tokens end with a separator token (102) and the sequenceis padded with 0 values to length 384.
        var allTokens: [Int] = []
        /// Would love to create it in a single Swift line but Xcode compiler fails...
        allTokens.append(
        ...
        print(allTokens.count)

        let input_ids = try! MLMultiArray(shape: [2, 11], dataType: .float16) // MLMultiArray.from(stacked, dims: 2)
        for i in 0..<11 {
            input_ids[[0, i] as [NSNumber]] = allTokens[i] as NSNumber
        }
        for i in 11..<22 {
            input_ids[[1, i - 11] as [NSNumber]] = allTokens[i] as NSNumber
        }
        
        let questionMask = Array(repeating: 1, count:tokensQuestion.count + 2)
        let paddingMask = Array(repeating: 0, count: nPadding)
        let attentionMaskArray = questionMask + paddingMask
        
        let attentionMask = try! MLMultiArray(shape: [2, 11], dataType: .float16)
        for i in 0..<11 {
            attentionMask[[0, i] as [NSNumber]] = attentionMaskArray[i] as NSNumber
        }
        for i in 11..<22 {
            attentionMask[[1, i - 11] as [NSNumber]] = attentionMaskArray[i] as NSNumber
        }
                        
//        return bert_classifierInput(sent_id: input_ids, mask_1: input_ids)
        return bert_classifierInput(word_id: input_ids)
    }
}
